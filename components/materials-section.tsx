...
export function MaterialsSection() {
  return (
    <section id="materiales" className="py-12 sm:py-16 lg:py-20">
      <div className="px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {materials.map((material, index) => (
            <div
              key={index}
              className="flex flex-col gap-4 p-4 sm:p-6 border border-gray-200 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-white"
            >
              <div
                className={`mb-4 inline-flex h-12 w-12 items-center justify-center rounded-full ${material.color}`}
              >
                {React.createElement(material.icon, { className: "h-6 w-6" })}
              </div>
              <h3 className="mb-2 text-lg font-semibold text-gray-900">{material.title}</h3>
              <p className="text-gray-600 text-sm mb-3">{material.description}</p>
              <div className="border-t pt-3">
                <p className="text-xs text-green-600 font-medium">💡 {material.tip}</p>
              </div>
            </div>
          ))}
        </div>
